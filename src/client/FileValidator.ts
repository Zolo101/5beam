...
function processLevel(result: ValidateResult, level: string, id: number): DetectedLevel {
    ...
    if (background < 0 || background > 15)
        logs.push(
            createError(id, `The background index given: '${background}' is not between 0 and 15`)
        );
    if (type != "L" && type != "H")
        logs.push(createError(id, `The level type is '${type}' which is not 'L' or 'H'`));
        ...
        case "L":
            if (levelData.length !== width * height)
                logs.push(
                    createError(
                        id,
                        `'${name}' level data size is not the same as the specified size (width, height) in the level properties`
                    )
                );
            break;

        case "H":
            if (levelData.length !== width * height * 2)
                logs.push(
                    createError(
                        id,
                        `'${name}' level data size is not the same as the specified size (width, height) in the level properties`
    ...
    if (levelData.indexOf(":") === -1)
        logs.push(createLog(id, `A win-token is optional but recommended`, "warning"));
    ...
    if (Number.isNaN(dialogueLength))
        logs.push(
            createError(
                id,
                `Dialogue length number is missing! If your level has zero dialogue, just add "00" above the required deaths number.`
            )
        );
    const dialogues = lines.slice(
        2 + height + spriteNumber + 1,
        2 + height + spriteNumber + 1 + dialogueLength
    );
    if (dialogueLength !== dialogues.length)
        logs.push(
            createError(
                id,
                `The dialogue length number does not match the amount of dialogue lines in the level file!`
            )
        );
    ...
    if (Number.isNaN(deathsRequired))
        logs.push(
            createError(
                id,
                `The deaths required to beat this level is missing! If your level can be beaten with zero deaths, just add "000000" to the end of your level file.`
            )
        );
...
function processSprites(logs: ValidateLog[], sprites: string[], id: number): Sprite[] {
    ...
    for (const sprite of sprites) {
        ...
        if (motionSpeedAndString) {
            ...
            if (motionSpeed < 0 || motionSpeed > 99)
                logs.push(
                    createError(
                        id,
                        `Sprite ${i} has a motion speed of ${motionSpeed} which is not between 0 and 99`
                    )
                );
        }
        ...
        if (Number.isNaN(entityId))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an entity id of '${spriteProps[0]}' which is not a number`
                )
            );
        if (Number.isNaN(x))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an x coordinate of '${spriteProps[1]}' which is not a number`
                )
            );
        if (Number.isNaN(y))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an y coordinate of '${spriteProps[2]}' which is not a number`
                )
            );
        if (Number.isNaN(roleId))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an role id of '${roleIdString}' which is not a number`
                )
            );
        if (Number.isNaN(motionSpeed))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an motion speed of '${motionSpeed}' which is not a number`
                )
            );
        if (Number.isNaN(motionPath))
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an motion path of '${motionPath}' which is not a number`
                )
            );

        if (entityId < 0 || entityId > 99)
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has an entity id of ${entityId} which is not between 0 and 99`
                )
            );
        ...
        if (roleId < 0 || roleId > 99)
            logs.push(
                createError(
                    id,
                    `Sprite ${i} has a role id of ${roleId} which is not between 0 and 99`
                )
            );
    ...
    for (const line of dialogue) {
        ...
        if (entityId < 0 || entityId > 99)
            logs.push(
                createError(
                    id,
                    `Dialogue ${i} has an entity id of ${entityId} which is not between 0 and 99`
                )
            );
        if (emotion != "H" && emotion != "S")
            logs.push(
                createError(
                    id,
                    `Dialogue ${i} has an emotion of '${emotion}' which is not 'H' or 'S'`
                )
            );
