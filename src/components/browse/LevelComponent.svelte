<script lang="ts">
    ...
    let { level = $bindable(), glow = false }: Props = $props();

    const { creator, id, modded, title, featured, difficulty, plays } = level;
    let thumbnailUrl = $derived(getLevelThumbnailURL(level.id, level.thumbnail, true));
