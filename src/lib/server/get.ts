...
/** @deprecated */
// export async function getLevelByIdLoggedIn(id: string) {
//     return weirdAhhQuery(
//         async () => {
//             const level = await levels.getOne<Level>(id, { expand: "creator" });
//             const starred = await hasUserStarred({ id });
...
/** @deprecated */
// export async function getLevelpackByIdLoggedIn(id: string) {
//     return weirdAhhQuery(
//         async () => {
//             const levelpack = await levelpacks.getOne<Level>(id, { expand: "creator" });
//             const starred = await hasUserStarred({ id });
...
//     const filter = clientPb.filter(`creator = {:id}`, { id });
...
//     return (await collection.getList(1, 9999, { filter, ...options })).map((v) => v.item);
// }
